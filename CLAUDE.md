...
### 测试
项目目前没有任何自动化测试文件（`*_test.go`）。手动 API 测试用例文档位于 `docs/api-test/API_TEST_SPEC_BACKEND_V1.md`（约 1884 行）。
...
- **前端**: Vue 3 + Vite + Vue Router 5, Tailwind CSS 4 + daisyUI 5
...
│       ├── errors/               # （预留）错误处理
│       └── logger/               # （预留）结构化日志
├── frontend/                     # Vue 3 前端（主项目）
│   ├── vite.config.js
│   ├── tailwind.config.js
│   └── src/
│       ├── main.js / App.vue / main.css
│       ├── router/index.js       # 8 条路由，含 auth guard
│       ├── services/
...
│       └── components/           # 公共组件
├── frontend-study/               # 前期前端原型（gitignored，含 ProfilePage.vue 等额外页面）
...
| `/uploads` | 是 | POST create（目前为桩实现，返回 501） |
...
3. **日记 CRUD**: 所有日记操作按 `user_id` 隔离，用户只能操作自己的日记。PUT 为全量替换，PATCH 为部分更新。
...
5. **自动迁移**: 启动时 GORM AutoMigrate 自动同步数据库 schema，修改 Model 后无需手动迁移。
6. **无 session 机制**: 后端完全无状态，不存储 session。登出仅为前端清除 token 的占位操作。
...
| `/app/profile` | ProfilePage | 是 | ⚠️ 路由已定义但 `ProfilePage.vue` 文件不存在 |

### 已知问题

1. **前端路由 bug**: `router/index.js` 第 52 行，`/app/diaries/:id` 的组件错误地写成了 `DraftDetailPage`，实际应为 `DiaryDetailPage`。
2. **缺失 ProfilePage.vue**: `router/index.js` 导入了 `ProfilePage.vue`，但该文件仅存在于 `frontend-study/` 原型中，主 `frontend/` 项目尚未创建。
3. **`.env.example` 不存在**: README 中提到的示例文件尚未创建。

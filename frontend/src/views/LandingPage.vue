  ...
  <main class="cd-container pt-[70px]">
      <section class="flex flex-col gap-[40px] md:flex-row md:items-start">
        ...
        <div class="flex-1">
          <div class="relative h-[420px] w-full overflow-hidden rounded-[45px] border border-[#191A23] bg-[#F3F3F3]">
            ...
            <div class="absolute left-[120px] top-[150px] h-[150px] w-[150px] rounded-full border border-[#191A23] bg-white">
            </div>
            <div class="absolute left-[260px] top-[210px] h-[110px] w-[110px] rounded-full border border-[#191A23] bg-[#B9FF66]">
            </div>
          </div>
        </div>
      </section>
          ...
          <div class="cd-card flex items-center justify-between p-[50px]">
            <div class="flex flex-col justify-between gap-[40px]">
              ...
              <RouterLink class="cd-p inline-flex items-center gap-[15px]" to="/app/diaries/new">
                <span>开始</span>
                <span class="inline-flex h-[32px] w-[32px] items-center justify-center rounded-full border border-[#191A23] bg-white">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path d="M4 7H10" stroke="#191A23" stroke-width="1.5" stroke-linecap="round" />
                    <path d="M8 5L10 7L8 9" stroke="#191A23" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </span>
              </RouterLink>
            </div>
      ...
      <section id="plan" class="mt-[70px]">
        ...
        <div class="mt-[40px] grid gap-[15px]">
          <button v-for="(s, idx) in processSteps" :key="s.title" class="cd-card flex w-full items-center justify-between p-[30px] text-left" :class="processOpenIndex === idx ? '!bg-[#191A23] text-white !border-white/20' :'!bg-[#F3F3F3] text-[#191A23]'" type="button" @click="processOpenIndex = processOpenIndex === idx ? -1 : idx">
            <div>
            ...
            <div class="flex h-[40px] w-[40px] items-center justify-center rounded-full border " :class="processOpenIndex === idx ? 'border-white/40 bg-white/10 text-white' : 'border-[#191A23] bg-white text-[#191A23]'">
        ...
        <div class="mt-[40px] cd-card bg-[#F3F3F3] p-[50px]">
          <form class="grid gap-[20px] md:grid-cols-2" @submit.prevent>
            <label class="grid gap-[10px]">
              ...
              <input class="h-[54px] rounded-[14px] border border-[#191A23] bg-white px=[18px] outline-none" placeholder="怎么称呼你喵？"/>
            </label>
            <label class="grid gap-[10px]">
            ...
            <label class="grid gap-[10px] md:col-span-2">
              ...
              <textarea
                ...
                placeholder="比如：搜索、标签、统计、主题、同步、导出等等喵~"
              ></textarea>
            </label>
